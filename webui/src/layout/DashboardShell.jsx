...
const ConfigContainer = lazy(() => import('../features/config/ConfigContainer'))
...
export default function DashboardShell({ token, onLogout, config, fetchConfig, showMessage, message, onForceLogout }) {
    ...
    const navItems = [
        { id: 'config', label: t('nav.config.label'), icon: Cookie, description: t('nav.config.desc') },
    ...
    useEffect(() => {
        ...
        async function loadVersion() {
            try {
                ...
                if (!disposed) setVersionInfo(data)
            } catch (_err) {
                if (!disposed) setVersionInfo(null)
            }
        }
        loadVersion()
    ...
    const renderTab = () => {
        switch (activeTab) {
            case 'config':
                return <ConfigContainer config={config} onRefresh={fetchConfig} onMessage={showMessage} authFetch={authFetch} />
            case 'test':
        ...
        <div className="flex h-screen bg-background overflow-hidden text-foreground">
                ...
                <div className="p-4 border-t border-border bg-card">
                    <div className="space-y-4">
                        <div className="flex items-center justify-between text-sm px-1">
                            ...
                            <span className="flex items-center gap-1.5 text-[10px] font-bold text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="bg-background rounded-lg p-3 border border-border shadow-sm">
                                <div className="text-[9px] text-muted-foreground font-bold uppercase tracking-wider mb-0.5 opacity-70">{t('sidebar.cookies')}</div>
                                <div className="text-lg font-bold text-foreground leading-tight">{cookieCount}</div>
                            </div>
                            <div className="bg-background rounded-lg p-3 border border-border shadow-sm">
                                <div className="text-[9px] text-muted-foreground font-bold uppercase tracking-wider mb-0.5 opacity-70">{t('sidebar.keys')}</div>
                                <div className="text-lg font-bold text-foreground">{keyCount}</div>
                            </div>
                        </div>
                        <div className="bg-background rounded-lg p-3 border border-border shadow-sm">
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                <header className="lg:hidden h-14 flex items-center justify-between px-4 border-b border-border bg-card">
                    ...
                    <div className="flex items-center gap-2">
                        <LanguageToggle />
                        <button onClick={() => setSidebarOpen(true)} className="p-2 -mr-2 text-muted-foreground hover:text-foreground">
                ...
                <div className="flex-1 overflow-auto bg-background p-4 lg:p-10">
                    <div className="max-w-6xl mx-auto space-y-4 lg:space-y-6">
                        <div className="hidden lg:block mb-8">
                            <h1 className="text-3xl font-bold tracking-tight mb-2">{activeNavItem?.label}</h1>
                            <p className="text-muted-foreground">{activeNavItem?.description}</p>
                        </div>
                        {message && (
                            <div className={clsx(
                        ...
                        <div>
                            <Suspense fallback={<TabLoadingFallback label={activeNavItem?.label || 'Baidu2API'} />}>
                                {renderTab()}
                            </Suspense>
                        </div>
                    </div>
