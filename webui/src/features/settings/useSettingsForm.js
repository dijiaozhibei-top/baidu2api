...
const DEFAULT_FORM = {
    ...
    runtime: { account_max_inflight: 1, account_max_queue: 1, global_max_inflight: 1, token_refresh_interval_hours: 24 },
    ...
    context: { fresh_conversation: true, max_chars: 12000, max_messages: 16, max_message_chars: 2000 },
    model_aliases_text: '{}',
}
...
function fromServerForm(data) {
    ...
    return {
        ...
        context: {
            fresh_conversation: data.context?.fresh_conversation ?? true,
            max_chars: Number(data.context?.max_chars || 12000),
            max_messages: Number(data.context?.max_messages || 16),
            max_message_chars: Number(data.context?.max_message_chars || 2000),
        },
    }
}
...
function toServerPayload(form) {
    ...
    return {
        ...
        context: {
            fresh_conversation: form.auto_delete?.mode === 'single' || form.auto_delete?.mode === 'all' || Boolean(form.context?.fresh_conversation),
            max_chars: Number(form.context?.max_chars || 12000),
            max_messages: Number(form.context?.max_messages || 16),
            max_message_chars: Number(form.context?.max_message_chars || 2000),
        },
    }
}
...
export function useSettingsForm({ apiFetch, t, onMessage, onRefresh, onForceLogout, isVercel = false }) {
    ...
    const loadExportData = useCallback(async () => {
        try {
            ...
            setExportData({ json: JSON.stringify(data.config || data, null, 2), raw: data })
    ...
    const downloadExportFile = useCallback(async () => {
        ...
        if (!latest?.json) {
            ...
            latest = {
                json: JSON.stringify(loaded.config || loaded, null, 2),
                raw: loaded,
            }
        }
        ...
        const filename = `baidu2api-config-backup-${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}-${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}.json`
