        ...
        <div className="space-y-6">
            {autoFetchPaused && (
                <div className="p-4 rounded-lg border border-destructive/30 bg-destructive/10 text-destructive flex items-center justify-between gap-4">
                    ...
                    <button type="button" onClick={retryLoadSettings} className="px-3 py-1.5 text-xs rounded-md border border-destructive/40 hover:bg-destructive/10">
                        {t('settings.retryLoad')}
                    </button>
                </div>
            )}
            {settingsMeta.default_password_warning && (
                <div className="p-4 rounded-lg border border-blue-300/30 bg-blue-500/10 text-blue-700 flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4" />
            ...
            <ContextSection t={t} form={form} setForm={setForm} />
            ...
            <div className="flex justify-end">
                ...
                >
                    ...
                    {saving ? t('actions.loading') : (t('settings.save') || t('settings.saveSettings') || 'Save')}
                </button>
            </div>
        </div>
    )
